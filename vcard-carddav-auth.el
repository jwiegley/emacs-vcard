;;; vcard-carddav-auth.el --- CardDAV authentication handlers -*- lexical-binding: t; -*-
...
;;   (setq auth (vcard-carddav-auth-basic-create
;;               :username "user"
;;               :password "secret"))
;;
;;   ;; Get Authorization header
;;   (vcard-carddav-auth-get-header auth)
;;   => "Basic dXNlcjpzZWNyZXQ="
;;
;;   ;; Create Bearer token auth
;;   (setq auth (vcard-carddav-auth-bearer-create
;;               :token "abcdef123456"))
...
(defgroup vcard-carddav-auth nil
  "Authentication for CardDAV operations."
  :group 'vcard
  :prefix "vcard-carddav-auth-")

;;; Error conditions

(define-error 'vcard-carddav-auth-error "CardDAV authentication error")

;;; Base authentication class

(defclass vcard-carddav-auth ()
  ((type
    :initarg :type
    :initform nil
    :type symbol
...
(defclass vcard-carddav-auth-basic (vcard-carddav-auth)
  ((username
    :initarg :username
    :initform nil
    :type (or null string)
...
(cl-defmethod initialize-instance :after ((auth vcard-carddav-auth-basic) &rest _slots)
  "Initialize Basic Auth object AUTH."
  (oset auth type :basic))

(cl-defmethod vcard-carddav-auth-get-header ((auth vcard-carddav-auth-basic))
...
Returns string like \"Basic base64(username:password)\"."
  (let ((username (oref auth username))
    ...
    (unless (and username password)
      (signal 'vcard-carddav-auth-error
...
(cl-defmethod vcard-carddav-auth-valid-p ((auth vcard-carddav-auth-basic))
...
(cl-defmethod vcard-carddav-auth-refresh ((auth vcard-carddav-auth-basic))
...
;;;###autoload
(defun vcard-carddav-auth-basic-create (&rest args)
...
Example:
  (vcard-carddav-auth-basic-create
   :username \"user@example.com\"
   :password \"secret\")"
  (let ((username (plist-get args :username))
        (password (plist-get args :password)))
    (unless username
      (signal 'vcard-carddav-auth-error '("Username required for Basic Auth")))
    (unless password
      (signal 'vcard-carddav-auth-error '("Password required for Basic Auth")))
    (vcard-carddav-auth-basic
     :username username
     :password password)))

;;; Bearer Token Authentication

(defclass vcard-carddav-auth-bearer (vcard-carddav-auth)
  ((token
    :initarg :token
    :initform nil
    :type (or null string)
...
(cl-defmethod initialize-instance :after ((auth vcard-carddav-auth-bearer) &rest _slots)
  "Initialize Bearer Auth object AUTH."
  (oset auth type :bearer))

(cl-defmethod vcard-carddav-auth-get-header ((auth vcard-carddav-auth-bearer))
...
(cl-defmethod vcard-carddav-auth-valid-p ((auth vcard-carddav-auth-bearer))
...
(cl-defmethod vcard-carddav-auth-refresh ((auth vcard-carddav-auth-bearer))
...
;;;###autoload
(defun vcard-carddav-auth-bearer-create (&rest args)
...
Example:
  (vcard-carddav-auth-bearer-create
   :token \"abc123def456\"
   :expires-at (+ (float-time) 3600))"
  (let ((token (plist-get args :token))
        (expires-at (plist-get args :expires-at)))
    (unless token
      (signal 'vcard-carddav-auth-error '("Token required for Bearer Auth")))
    (vcard-carddav-auth-bearer
     :token token
     :expires-at expires-at)))

;;; OAuth2 Authentication (structure for future implementation)

(defclass vcard-carddav-auth-oauth2 (vcard-carddav-auth)
  ((client-id
    :initarg :client-id
    :initform nil
    :type (or null string)
...
(cl-defmethod initialize-instance :after ((auth vcard-carddav-auth-oauth2) &rest _slots)
  "Initialize OAuth2 Auth object AUTH."
  (oset auth type :oauth2))

(cl-defmethod vcard-carddav-auth-get-header ((auth vcard-carddav-auth-oauth2))
...
(cl-defmethod vcard-carddav-auth-valid-p ((auth vcard-carddav-auth-oauth2))
...
(cl-defmethod vcard-carddav-auth-refresh ((auth vcard-carddav-auth-oauth2))
...
;;;###autoload
(defun vcard-carddav-auth-oauth2-create (&rest args)
...
This is a structure for future implementation."
  (let ((client-id (plist-get args :client-id))
        ...
        (expires-at (plist-get args :expires-at)))
    (unless client-id
      (signal 'vcard-carddav-auth-error '("Client ID required for OAuth2")))
    (unless client-secret
      (signal 'vcard-carddav-auth-error '("Client secret required for OAuth2")))
    (unless token-url
      (signal 'vcard-carddav-auth-error '("Token URL required for OAuth2")))
    (vcard-carddav-auth-oauth2
     :client-id client-id
...
(defun vcard-carddav-auth-ensure-valid (auth)
...
Signals error if credentials cannot be validated."
  (unless (vcard-carddav-auth-valid-p auth)
    ;; Try to refresh
    (setq auth (vcard-carddav-auth-refresh auth))
    ;; Check again
    (unless (vcard-carddav-auth-valid-p auth)
      (signal 'vcard-carddav-auth-error
...
(provide 'vcard-carddav-auth)
;;; vcard-carddav-auth.el ends here
